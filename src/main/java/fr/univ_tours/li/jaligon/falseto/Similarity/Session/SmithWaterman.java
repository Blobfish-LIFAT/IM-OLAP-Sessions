...
public class SmithWaterman extends SessionComparison {
    ...
    */
    public int getBestSubsequenceLength(){
        CellInfoEditDistance endPoint = new CellInfoEditDistance(Integer.MIN_VALUE, 0, 0, EditDistanceOperatorType.NOOPERATION, 0);
        int topRow = 0, topColumn = 0;
        for(int i=1; i<row; i++){
            for(int j=1; j<column; j++){
                //Search for the highest similarity with a MATCH_MISMATCH operation
                if(costMatrix[i][j].getSimilarity() >= endPoint.getSimilarity() && costMatrix[i][j].getOperator().equals(EditDistanceOperatorType.MATCH_MISMATCH)) {
                    endPoint = new CellInfoEditDistance(costMatrix[i][j].getSimilarity(), i, j, 
                            EditDistanceOperatorType.NOOPERATION, costMatrix[i][j].getQuerySimilarity());
                    topRow = i;
                    topColumn = j;
                }
            }
    ...
    public CellInfoEditDistance getFirstMatchingQueryInBestSubsequence(){
        CellInfoEditDistance startPoint = new CellInfoEditDistance(Integer.MIN_VALUE, 0, 0, EditDistanceOperatorType.NOOPERATION, 0);
        int topRow = 0, topColumn = 0;
        for(int i=1; i<row; i++){
            for(int j=1; j<column; j++){
                //Search for the highest similarity with a MATCH_MISMATCH operation
                if(costMatrix[i][j].getSimilarity() >= startPoint.getSimilarity() && costMatrix[i][j].getOperator().equals(EditDistanceOperatorType.MATCH_MISMATCH)) {
                ...
                case MATCH_MISMATCH:
                    if(length>0)
                        startPoint = new CellInfoEditDistance(costMatrix[topRow][topColumn].getSimilarity(), topRow, topColumn, 
                                EditDistanceOperatorType.NOOPERATION, costMatrix[topRow][topColumn].getQuerySimilarity());
                    topRow--;
                    topColumn--;
                    length++;
                    break;
